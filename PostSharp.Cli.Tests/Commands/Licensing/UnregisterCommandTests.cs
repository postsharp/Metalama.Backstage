...
namespace PostSharp.Cli.Tests.Commands.Licensing
{
    public class UnregisterCommandTests : LicensingCommandsTestsBase
    {
        ...
        {
            await TestCommandAsync( $"license unregister 1", "", "Invalid ordinal." + Environment.NewLine, 1 );
        }

        [Fact]
        public async Task UnknownLicenseKeyFailsToUnregisterInCleanEnvironment()
        {
            await TestCommandAsync( $"license unregister {TestLicenses.Key1}", "", $"This license is not registered." + Environment.NewLine, 2 );
        }
        ...
        public async Task OneOrdinalUnregisters()
        {
            ...
            await TestCommandAsync( $"license unregister 1", $"{TestLicenses.Key1} unregistered." + Environment.NewLine );
        ...
        {
            await TestCommandAsync( $"license register {TestLicenses.Key1}", "" );
            await TestCommandAsync( $"license unregister {TestLicenses.Key1}", $"{TestLicenses.Key1} unregistered." + Environment.NewLine );
            await TestCommandAsync( "license list", "" );
        }
